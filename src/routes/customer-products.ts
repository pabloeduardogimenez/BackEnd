...
export default async function customerProductsRoutes(fastify: FastifyInstance) {
    ...
    fastify.post("/:customerId/favorites", async (request, reply)=>{
        ...
        try{
            ...
            //_id  mesmo campo do Mongo ou name de produtos
            ...
            console.error("error ap processa requisiação:",error);
            reply.code(500);
            return {
                startCode : 500,
                error: "Erro ao Processar requisisão",
                messagen: "Ocorreu um erro ao processar a requisisão",
                datails: error instanceof Error ? error.message : "Erro desconhecido",
            }
        }
    });
